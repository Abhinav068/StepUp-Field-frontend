...
let loginUserToken = JSON.parse(localStorage.getItem("loginUserToken")) || false;
...
async function razorpay() {
  let amount = (+totalprices)*100; //Get from cart value (in paise);

  console.log('payment process started...',totalprices);

  let response = await fetch(`${URL}/payments/create-payment`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
...
async function verifypayment(obj){
  ...
  let response = await fetch(`${URL}/payments/verifyOrder`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
